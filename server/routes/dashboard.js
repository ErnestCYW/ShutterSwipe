...
router.get("/", authorization, async (req, res) => {
  try {
    ...
    const pic_repo = await pool.query(
      "SELECT pic_id FROM pics  WHERE user_id = $1 ORDER BY created_date DESC",
      [req.user]
    );
...
router.post("/upload", authorization, async (req, res) => {
  ...
  const new_pic_id = await pool.query(
    "INSERT INTO pics (pic_id, user_id, pic_score, created_date) VALUES (DEFAULT, $1, DEFAULT, NOW()) RETURNING pic_id",
    [req.user]
  );

  //rename file to UUID and transfer from client to picture_server, stores as jpg

  if (new_pic_id.rows.length === 1) {
    file.mv(
...
router.get("/getPics", authorization, async (req, res) => {
  const updatedPics = await pool.query(
    "SELECT pic_id FROM pics WHERE user_id = $1",
    [req.user]
  );

  const toReturn = {
    updatedPicRepo: JSON.stringify(updatedPics.rows),
  };

  res.json(toReturn);
});

// delete pictures route
